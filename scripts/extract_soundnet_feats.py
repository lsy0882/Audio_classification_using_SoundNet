...
def extract_features(args):
    ...
    for sound_sample, audio_path in gen_audio_from_dir(args.input_dir,
        ...
        # Save conv7 features
        conv7_feats = feats['conv7'].squeeze().mean(1).cpu().data.numpy()
        conv7_output_dir = Path(args.output_dir, 'conv7')
        conv7_output_dir.mkdir(parents=True, exist_ok=True)
        np.savetxt(conv7_output_dir / f'{Path(audio_path).stem}.csv', conv7_feats, delimiter=';')
